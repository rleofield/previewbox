...
namespace rlf {
   ...
   namespace rimg_math {
      ...
      void rimginterface::Gauss31( tImgPlanar& img ) {
         ...
         for( iy = 0; iy < sy; iy++ ) {
            for( ix = 0; ix < sx; ix++ ) {
               img[0][ iy ][ix] = ( uint8_t )( ( *bword ) >> shift );
               bword++;
            }
      ...
      void rimginterface::Gauss( tImgPlanar& img, int ker_size ) {
         ...
         for( int i = 0; i < times; i++ ) {
            ...
            while( ycount-- ) {
               ...
               while( xcount-- ) {
                  ...
                  *ptr_target++ = ( uint32_t )( *( ptr_source - 1 )    // 1
         ...
         for( iy = 0; iy < sy; iy++ ) {
            for( ix = 0; ix < sx; ix++ ) {
               img[0][ iy ][ix] = ( uint8_t )( ( unsigned long )( ( *bdword ) >> shift ) );
               bdword++;
            }

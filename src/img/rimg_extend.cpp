...
namespace rlf {
   ...
   namespace rimg_math {
      ...
      void extend_lines_at_all_sides( tPlane8& image, size_t half ) {
         size_t s = half;

         size_t xend = image[0].size();
         size_t x_intern_end = xend - s;
         size_t x_intern_last = x_intern_end - 1;

         size_t yend = image.size();
         size_t y_intern_end = yend - s;
         size_t y_intern_last = y_intern_end - 1;


         // first s lines
         for( size_t yborder = 0; yborder < s; yborder++ ) {
         ...
         for( size_t xborder = 0; xborder < s; xborder++ ) {
         ...
         for( size_t y = s; y < y_intern_end; y++ ) {
      ...
      void extend_lines_at_all_sides( tImgPlanar& image, size_t half ) {
      ...
      void rimginterface::ExtendImage( const tImgViewPlanar& in, tImgPlanar& extended, size_t kernel ) {
